Sia $P = (Q, \Sigma, \Gamma, \delta, q_0, Z_0, F)$ un [[Automi a Pila (PDA)|PDA]].
...
dove:
- questa e' una [[Configurazione di un PDA]]
- $q$ e' lo stato attuale dell'automa
- $aw$ e' la porzione di stringa non ancora consumata, dove $a$ e' un simbolo. E' proprio $a$ a condizione la scelta del caso della delta
...
- $\alpha\beta$ perche' la [[funzione di transizione degli stati per automi a pila non deterministici]], ritorna come simboli di cui fare la [[push nei pda|push]] $\alpha$, ha consumato il primo simbolo in cima, ma rimane da tenere da parte ancora $\beta$
## Osservazioni
- il simbolo $\vdash$ rappresenta una relazione binaria tra coppie di [[Configurazione di un PDA|configurazioni]]