...
- BASE: $I \vdash^* I$ per ogni $ID \ di \ I$ (ID e' la [[Configurazione di un PDA|descrizione istantanea]])
...
# Teorema 1:
Se $P = (Q,\Sigma, \Gamma, \delta, q_0, Z_0, F)$ e' un [[Automi a Pila (PDA)]], e $(q, x, \alpha) \vdash^* (p, y, \beta$) allora $\forall w \in \Sigma^*$ e $\forall \gamma \in \Gamma^*$ vale che :
...
$$ (q, x, \alpha) \vdash^* (p, y, \beta)$$
- per cio' che riguarda le stringhe in input, il teorema in questa forma potrebbe ancora valere, perche' come detto prima l'[[Automi a Pila (PDA)]] vede solo il primo simbolo in input pertanto w non viene toccata durante la computazione 
...
## Teorema 2:
Se $P = (Q,\Sigma, \Gamma, \delta, q_0, Z_0, F)$ e' un [[Automi a Pila (PDA)]], e $(q, xw, \alpha) \vdash^* (p, yw, \beta)$ allora vale che:
$$(q, x, \alpha) \vdash^* (p, y, \beta)$$
